...
function updateLineCharts() {
  ...
  const activeTowns = d3
    .selectAll("#townGroup")
    .selectAll("path[active='true']");
  ...
  const locations = Object.keys(data)
    ...
    .flat();
  const color = d3.scaleOrdinal().domain(locations).range(d3.schemeSet2);
  ...
  const y = d3
    .scaleSymlog()
    .domain(
      d3.extent(dataReady.map((data) => data.values.map((d) => d.new)).flat()),
    )
    .range([height, 0]);
  g.selectAll("#yAxis").transition().duration(duration).call(d3.axisLeft(y));
  ...
  const path = g.selectAll("#lines").selectAll("path").data(dataReady);
  path
    .enter()
    .append("path")
    .merge(path)
  ...
  const circleGroup = g.selectAll("#dots").selectAll("g").data(dataReady);
  ...
  const label = g.selectAll("#labels").selectAll("text").data(dataReady);
  ...
  label
    ...
    .merge(label)
    .datum((d) => {
      return { location: d.location, value: d.values[d.values.length - 2] };
